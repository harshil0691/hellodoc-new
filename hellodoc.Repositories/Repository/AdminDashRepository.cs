...
namespace hellodoc.Repositories.Repository
{
    public class AdminDashRepository : IAdminDashRepository
    {
        ...
        { 
            ...
            if(regionid == 0 && search == null)
            {
                admin.adminDashModels = requestlist.Skip((page - 1) * pagesize).Take(pagesize).ToList();
                admin.requestcount = requestlist.Count();
            }
            else if(regionid != 0 && search == null)
            {
                admin.adminDashModels = requestlist.Where(u => u.regionid == regionid).Skip((page - 1) * pagesize).Take(pagesize).ToList();
                admin.requestcount = requestlist.Where(u => u.regionid == regionid).Count();
            }
            else if(regionid == 0 && search != null)
            {
                admin.adminDashModels = requestlist.Where(u => u.PatientName.Contains(search)).Skip((page - 1) * pagesize).Take(pagesize).ToList();
                admin.requestcount = requestlist.Where(u => u.PatientName.Contains(search)).Count();
            }
            else
            {
                admin.adminDashModels = requestlist.Where(u => u.PatientName.Contains(search)&& u.regionid == regionid).Skip((page - 1) * pagesize).Take(pagesize).ToList();
                admin.requestcount = requestlist.Where(u => u.PatientName.Contains(search) && u.regionid == regionid).Count();
            }
